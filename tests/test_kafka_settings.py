...
def test_defaults_are_defined() -> None:
    ...
    assert DEFAULT_TOPIC == "product-sales-case"
    assert DEFAULT_GROUP_ID == "streaming-consumer-group-A"
    assert DEFAULT_AUTO_OFFSET_RESET == "earliest"
    assert DEFAULT_CLEAR_TOPIC_ON_START is False
...
def test_from_env_uses_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("KAFKA_CLEAR_TOPIC_ON_START", raising=False)
    ...
    assert settings.clear_topic_on_start is DEFAULT_CLEAR_TOPIC_ON_START
...
def test_from_env_reads_env_vars(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("KAFKA_CLEAR_TOPIC_ON_START", "true")
    ...
    assert settings.clear_topic_on_start is True
...
def test_from_env_reads_clear_topic_true_values(
    monkeypatch: pytest.MonkeyPatch,
    env_value: str,
) -> None:
    monkeypatch.setenv("KAFKA_CLEAR_TOPIC_ON_START", env_value)
...
def test_from_env_reads_clear_topic_false_values(
    monkeypatch: pytest.MonkeyPatch,
    env_value: str,
) -> None:
    monkeypatch.setenv("KAFKA_CLEAR_TOPIC_ON_START", env_value)
    ...
    assert settings.clear_topic_on_start is False


def test_from_env_rejects_invalid_clear_topic_value(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("KAFKA_CLEAR_TOPIC_ON_START", "maybe")

    with pytest.raises(ValueError, match="KAFKA_CLEAR_TOPIC_ON_START"):
...
def test_producer_config_has_required_keys() -> None:
    settings = KafkaSettings()
    config = settings.producer_config()

    assert config["bootstrap.servers"] == DEFAULT_BOOTSTRAP_SERVERS
    assert config["broker.address.family"] == DEFAULT_BROKER_ADDRESS_FAMILY
    assert config["log_level"] == "3"
    assert config["message.timeout.ms"] == "5000"
    assert config["request.timeout.ms"] == "5000"
    assert config["socket.timeout.ms"] == "5000"


def test_consumer_config_has_required_keys() -> None:
    settings = KafkaSettings()
    config = settings.consumer_config()

    assert config["bootstrap.servers"] == DEFAULT_BOOTSTRAP_SERVERS
    assert config["broker.address.family"] == DEFAULT_BROKER_ADDRESS_FAMILY
    assert config["group.id"] == DEFAULT_GROUP_ID
    assert config["auto.offset.reset"] == DEFAULT_AUTO_OFFSET_RESET
    assert config["log_level"] == "3"


def test_broker_address_family_v6_propagates_to_configs() -> None:
    settings = KafkaSettings(broker_address_family="v6")
